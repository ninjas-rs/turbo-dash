...
function ClaimModal({
  ...
  setActiveToast,
  setToasts,
}: ClaimModalProps): JSX.Element | null {
  const { isActive } = useCapsuleStore();
  const [contests, setContests] = useState<Contest[]>([]);
  const [ethPrice, setEthPrice] = useState<number | null>(null);
  const [loadingContests, setLoadingContests] = useState<
  ...
  const handleClaim = async (
    ...
    try {
      ...
      setContests((prev) =>
        prev.filter((c) => c.contestPubKey !== contestPubKey),
      );
    } catch (err) {
  ...
  const userContests = contests.filter(
    (contest) => contest.leader.address === signer.address,
  );
          ...
          <div className="flex flex-col space-y-2 w-full max-h-[60vh] overflow-y-auto">
            {isInitialLoading ? (
              ...
              <div className="text-center py-4">
                No contests available to claim
              </div>
            ) : (
                ...
                >
                  <div className="grid grid-cols-3 w-full items-center px-2">
                    ...
                    <span className="text-right tabular-nums">
                      {ethPrice
                        ? `${(contest.prizePool * ethPrice).toFixed(3)} $`
                        : `${contest.prizePool} ETH`}
                    </span>
                  </div>
                  {contest.status === "Ended" && (
                    <Button
                      ...
                      onClick={() =>
                        handleClaim(contest.roundId, contest.contestPubKey)
                      }
                      disabled={loadingContests[contest.contestPubKey]}
                    >
                      {loadingContests[contest.contestPubKey]
                        ? "Claiming..."
                        : "Claim"}
                    </Button>
                  )}
