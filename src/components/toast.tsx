  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-grow min-w-0">
        ...
        <p
          className={`text-xs font-mono truncate ${status === "pending" ? "text-yellow-700" : "text-green-700"}`}
        >
          {shortSignature}
        </p>
        {status !== "pending" && (
      ...
      setToasts((prev) =>
        prev.filter((toast) => {
          ...
          if (
            ...
            !pendingSignatures.has(toast.signature)
          ) {
            return false;
          }
    ...
    <div className="fixed top-4 left-4 z-50">
      <div className="flex flex-col">
        {toasts.map((toast, index) => (
          ...
          >
            <SingleToast signature={toast.signature} status={toast.status} />
          </div>
        ))}
      </div>
    </div>
